  ...
  static List<CalendarEvent> expandOccurrences({
    ...
    if (frequency == 'WEEKLY' && byDayIndexes.isNotEmpty) {
      ...
      while (baseWeekStart.isBefore(iterateEndLimit.add(duration))) {
        for (final weekday in byDayIndexes.sorted()) {
          final candidate = baseWeekStart.add(Duration(days: weekday - DateTime.monday, hours: event.start.hour, minutes: event.start.minute));
          if (candidate.isBefore(event.start)) {
            continue;
          }
          if (candidate.isAfter(iterateEndLimit)) {
            continue;
          }
          if (count != -1 && generated >= count) {
            break;
          }
    ...
    if (occurrences.isEmpty && !isException(event.start) &&
        event.start.isBefore(to) && !event.end.isBefore(from)) {
