...
class HouseholdRepository {
  ...
  Stream<List<Household>> watchHouseholds(String userId) {
    ...
    return query.snapshots().asyncMap((snapshot) async {
      final futures = snapshot.docs.map((membershipDoc) async {
        ...
        final householdDoc =
            await _households.doc(membership.householdId).get();
  ...
  Future<Household?> joinWithInvite({
    ...
    if (!inviteDoc.exists) {
      throw StateError('Einladung wurde nicht gefunden.');
    }
    ...
    if (!householdDoc.exists) {
      throw StateError('Haushalt existiert nicht mehr.');
    }
