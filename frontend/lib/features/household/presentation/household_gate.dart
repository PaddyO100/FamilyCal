...
class _HouseholdGateState extends State<HouseholdGate> {
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<List<Household>>(
      stream: _repository.watchHouseholds(widget.user.uid),
      builder: (context, snapshot) {
        ...
        final households = snapshot.data ?? <Household>[];
        if (households.isEmpty) {
          return HouseholdSelectPage(user: widget.user);
        }

        return CalendarShellPage(
          user: widget.user,
