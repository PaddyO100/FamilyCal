...
class _EventEditorSheetState extends State<EventEditorSheet> {
  ...
  void initState() {
    ...
    _selectedParticipants = List<String>.from(
      widget.initialEvent?.participantIds ?? <String>[],
    );
    _reminders = List<int>.from(widget.initialEvent?.reminderMinutes ?? <int>[30]);
    ...
    if (widget.initialEvent != null) {
      ...
      if (widget.initialEvent!.recurrenceRule != null) {
        final parsed = RecurrenceUtils.parseRule(widget.initialEvent!.recurrenceRule!);
        ...
        switch (freq) {
          ...
          default:
            ...
            _customRecurrenceController.text = widget.initialEvent!.recurrenceRule!;
            break;
        }
      }
    } else {
      ...
      if (currentUid != null) {
        _selectedParticipants = [currentUid];
      }
    }
  }
    ...
    switch (_recurrenceType) {
      ...
      case RecurrenceType.weekly:
        if (_weeklyWeekdays.isEmpty) {
          final weekday = _weekdayCode(_start.weekday);
          return 'FREQ=WEEKLY;INTERVAL=$_interval;BYDAY=$weekday';
        }
        final codes = _weeklyWeekdays.map(_weekdayCode).join(',');
  ...
  Future<void> _saveEvent(HouseholdCalendar calendar, List<Membership> members) async {
            ...
            return StreamBuilder<List<Membership>>(
              ...
              builder: (context, membershipSnapshot) {
                ...
                _selectedParticipants = _selectedParticipants
                    .where((participant) =>
                        members.any((member) => member.userId == participant))
                    .toList();
                  ...
                  child: ListView(
                    ...
                    children: [
                      ...
                      ListTile(
                        ...
                        title: Text(
                            'Beginn: ${MaterialLocalizations.of(context).formatMediumDate(_start)} ${MaterialLocalizations.of(context).formatTimeOfDay(TimeOfDay.fromDateTime(_start))}'),
                        onTap: () => _pickDateTime(isStart: true),
                      ),
                      ListTile(
                        leading: const Icon(Icons.timer_outlined),
                        title: Text(
                            'Ende: ${MaterialLocalizations.of(context).formatMediumDate(_end)} ${MaterialLocalizations.of(context).formatTimeOfDay(TimeOfDay.fromDateTime(_end))}'),
                      ...
                      Wrap(
                        spacing: 8,
                        children: [
                          for (final minutes in const [5, 10, 15, 30, 60, 120])
                            FilterChip(
                              label: Text('$minutes Min vorher'),
