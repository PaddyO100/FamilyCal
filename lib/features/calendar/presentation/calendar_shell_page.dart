...
class _CalendarShellPageState extends State<CalendarShellPage>{
  ...
  @override void initState(){ super.initState(); _selectedHousehold = widget.households.first; _membershipStream = _membershipForHousehold(_selectedHousehold.id); }
  Stream<Membership?> _membershipForHousehold(String hid){ return FirebaseFirestore.instance.collection('memberships').doc('${hid}_${widget.user.uid}').snapshots().map((s)=> s.exists? Membership.fromFirestore(s): null); }
  void _switchHousehold(Household h){ setState((){ _selectedHousehold = h; _currentIndex = 0; _membershipStream = _membershipForHousehold(h.id); }); }
  @override Widget build(BuildContext context){ return StreamBuilder<Membership?>(stream: _membershipStream, builder:(c,snap){ final membership = snap.data; final pages = [ MonthView(household: _selectedHousehold), WeekView(household: _selectedHousehold), DayView(household: _selectedHousehold), AgendaView(household: _selectedHousehold), AvailabilityView(household: _selectedHousehold, user: widget.user), BirthdayTab(household: _selectedHousehold) ]; return Scaffold(
    appBar: AppBar(title: Text(_selectedHousehold.name), actions:[ IconButton(onPressed: membership==null? null: (){ Navigator.of(context).push(MaterialPageRoute(builder: (c)=> SettingsPage(household: _selectedHousehold, user: widget.user, membership: membership))); }, icon: const Icon(Icons.settings_outlined)), PopupMenuButton<Household>(onSelected: _switchHousehold, itemBuilder: (c)=> widget.households.map((h)=> PopupMenuItem(value:h, child: Text(h.name))).toList(), child: Chip(avatar: const Icon(Icons.home_outlined, size:18), label: Text(_selectedHousehold.name, style: const TextStyle(fontWeight: FontWeight.w600)))), IconButton(onPressed: FirebaseAuth.instance.signOut, icon: const Icon(Icons.logout), tooltip: 'Abmelden') ]),
